...
{
    resize(850, 550);
    setWindowTitle(tr("Doubloon") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to an Doubloon address"));
    ...
    historyAction->setToolTip(tr("Check yer plunder history"));
    ...
    quitAction->setToolTip(tr("Abandon Ship"));
    quitAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_Q));
    quitAction->setMenuRole(QAction::QuitRole);
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About Doubloon"), this);
    aboutAction->setToolTip(tr("Show information about Doubloon"));
    ...
    optionsAction->setToolTip(tr("Modify configuration options for Doubloon"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Doubloon client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Doubloon client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Doubloon network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Doubloon address or malformed URI parameters."));
    }
...
void BitcoinGUI::backupWallet()
{
    ...
    if(!filename.isEmpty()) {
        if(!walletModel->backupWallet(filename)) {
            QMessageBox::warning(this, tr("Yarrr!"), tr("There was an error trying to save the wallet data to the new location."));
        }
    }
}
